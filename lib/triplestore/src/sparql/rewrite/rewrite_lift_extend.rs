...
pub fn rewrite_lift_extend(gp: GraphPattern) -> (GraphPattern, Vec<(Expression, Variable)>) {
    match gp {
        ...
        GraphPattern::Path { path , subject, object} => {
            (GraphPattern::Path { path , subject, object }, Vec::new())
        }
        GraphPattern::Join { left, right } => {
            ...
            (GraphPattern::Join { left:Box::new(left), right:Box::new(right) }, left_extends)
        }
        GraphPattern::LeftJoin { left, right, expression } => {
            let (left, mut left_extends) = rewrite_lift_extend(*left);
            let (right, right_extends) = rewrite_lift_extend(*right);
            left_extends.extend(right_extends);
            (GraphPattern::LeftJoin { left:Box::new(left), right:Box::new(right), expression }, left_extends)
        }
        GraphPattern::Filter { inner, expr } => {
            ...
            (GraphPattern::Filter {inner:Box::new(inner), expr}, keep_extends)
        }
        ...
        GraphPattern::Graph { name, inner } => {
            ...
            (GraphPattern::Graph {name, inner:Box::new(inner)}, extends)
        }
        ...
        GraphPattern::Values { variables, bindings } => {
            (GraphPattern::Values {variables, bindings}, Vec::new())
        }
        GraphPattern::PValues { variables, bindings_parameter } => {
            (GraphPattern::PValues {variables, bindings_parameter}, Vec::new())
        }
        ...
        GraphPattern::Distinct { inner } => {
            (GraphPattern::Distinct {inner:Box::new(terminating_lift_extend(*inner))}, Vec::new())
        }
        GraphPattern::Reduced { inner } => {
            (GraphPattern::Reduced {inner:Box::new(terminating_lift_extend(*inner))}, Vec::new())
        }
        GraphPattern::Slice { inner, start, length, } => {
            (GraphPattern::Slice {inner:Box::new(terminating_lift_extend(*inner)), start, length}, Vec::new())
        }
        GraphPattern::Group { inner, aggregates, variables } => {
            (GraphPattern::Group {inner:Box::new(terminating_lift_extend(*inner)), aggregates, variables}, Vec::new())
        }
        GraphPattern::Service { name, inner, silent } => {
            (GraphPattern::Service {name, inner, silent}, Vec::new())
        }
    }
}
