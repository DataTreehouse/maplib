...
pub fn create_compatible_cats(
    ...
    mut states: Vec<RDFNodeState>,
    global_cats: Arc<Cats>,
) -> Vec<HashMap<BaseRDFNodeType, (Vec<Expr>, BaseCatState)>> {
    let base_types: HashSet<_> = states.iter().map(|x| x.map.keys()).flatten().collect();
    ...
    exploded
        .into_iter()
        .map(|x| x.into_iter().map(|(e, t, s)| (t, (e, s))).collect())
        .collect()
}

fn check_need_native_cat_cast(t: &BaseRDFNodeType, types: &Vec<RDFNodeState>) -> bool {
...
fn check_need_string_cast(t: &BaseRDFNodeType, types: &Vec<RDFNodeState>) -> bool {
...
fn find_all_locals(t: &BaseRDFNodeType, states: &Vec<RDFNodeState>) -> Vec<Arc<Cats>> {
    let mut locals = vec![];
    for s in states {
