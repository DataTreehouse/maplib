...
//Based on the spargebra parser by Thomas Tanon
//Apache License, Version 2.0, (LICENSE-APACHE or http://www.apache.org/licenses/LICENSE-2.0)
//See lib/spargebra for a copy.
...
parser! {
    ...
    grammar parser(state: &mut ParserState) for str {
        pub rule StottrDocumentInit() -> StottrDocument = _ PrefixDeclList() _ statements:StottrStatements() _ {
            StottrDocument {
                directives: vec![],statements,prefix_map: state.namespaces.iter().map(|(x,y)|(x.clone(),NamedNode::new_unchecked(y))).collect()}
        }

        rule PrefixDeclList() = PrefixDecl() ** (_) {}

        rule PrefixDecl() = PrefixDeclNormal() / SparqlPrefixDecl() {}

        rule PrefixDeclNormal() = i("@PREFIX") _ ns:PNAME_NS() _ i:IRIREF() _ trailing_dot:"."? {
            if trailing_dot.is_none() {
                println!("Declaration of prefix {} is missing the period \".\"", ns)
            }
            state.namespaces.insert(ns.into(), i.into_inner());
        }

        rule SparqlPrefixDecl() = i("PREFIX") _ ns:PNAME_NS() _ i:IRIREF() _ trailing_dot:"."?{
            println!("Declaration of prefix {} should be done with @prefix syntax", ns);
            state.namespaces.insert(ns.into(), i.into_inner());
        }
        ...
        rule StottrArgumentList() -> Vec<Argument> = args:StottrArgument() ** (_ "," _) {
            args
        }
        ...
        rule StottrTermList() -> Vec<StottrTerm> = "(" _ ts:StottrTerm() ** (_ "," _) _ ")" {
            ts
        }
        ...
        rule StottrPatternList() -> Vec<Instance> = ts:StottrInstance() ** (_ ","? _) {
            ts
        }
        ...
        rule StottrAnnotationList() -> Vec<Annotation> = ans:StottrAnnotation() ** (_ "," _) {
            ans
        }
        ...
        rule StottrParameterList() -> Vec<Parameter> = ts:StottrParameter() ** (_ "," _) {
            ts
        }
        ...
        rule StottrParameter() -> Parameter = p:StottrParameterNonOptional() {
            ...
            p
        } / p:StottrParameterOptionalOrderReversed() {
            p
        }

        rule StottrParameterNonOptional() -> Parameter = nbl:"!"? _ pt:StottrPType()? _ v:Var() _ def:StottrDefaultValue()?  {
            Parameter {
                optional: false, non_blank:nbl.is_some(), ptype: pt, variable: v,default_value: def}
        }

        rule StottrParameterOptional() -> Parameter = "?" _ nbl:"!"? _ pt:StottrPType()? _ v:Var() _ def:StottrDefaultValue()?  {
            Parameter {
                optional: true, non_blank:nbl.is_some(), ptype: pt ,variable: v,default_value: def}
        }

        rule StottrParameterOptionalOrderReversed() -> Parameter = nbl:"!"? _ "?" _ pt:StottrPType()? _ v:Var() _ def:StottrDefaultValue()?  {
            Parameter {
                optional: true, non_blank:nbl.is_some(), ptype: pt ,variable: v,default_value: def}
        }
        ...
        rule StottrConstantTermOrList() -> ConstantTermOrList = ts:StottrConstantTermList() {
            ConstantTermOrList::ConstantList(ts)
        } / t:StottrConstantTerm() {
            ConstantTermOrList::ConstantTerm(t)
        }

        rule StottrConstantTermList() -> Vec<ConstantTermOrList> = "(" _ ts:StottrConstantTermOrList() ** (_ "," _) _ ")" {
            ts
        }
