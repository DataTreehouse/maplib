...
pub fn multi_columns_to_string_cols(
    ...
    for (c, t) in rdf_node_types {
        if let RDFNodeType::MultiType(types) = t {
            ...
            for t in types {
                to_coalesce.push(match t {
                    BaseRDFNodeType::IRI => (lit("<")
                        ...
                        + lit(">"))
                    .alias(c),
                    BaseRDFNodeType::BlankNode => (lit("_:")
                        + col(c)
                            .struct_()
                            .field_by_name(MULTI_BLANK_DT)
                            .cast(DataType::String))
                    ...
                    BaseRDFNodeType::Literal(l) => {
                        if t.is_lang_string() {
                            lang_string_lit_to_string_expr(c).alias(c)
            ...
            lf = lf.with_column(lang_string_lit_to_string_expr(c).alias(c));
        }
    }
    lf
}

pub fn lang_string_lit_to_string_expr(c: &str) -> Expr {
    lit("\"")
        + col(c)
            .struct_()
            .field_by_name(LANG_STRING_VALUE_FIELD)
